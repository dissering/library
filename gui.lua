do --// UI Source
    ...
    Library = {
        ...
        do
            ...
            Library.PlayerCard = function(Self, Params)
                Params = Params or { }

                local Card = {
                    Name = Params.Name or Params.name or "Player",
                    UserId = Params.UserId or 0,
                    Callback = Params.Callback or Params.callback or function() end,
                    Window = Self.Window,
                    Page = Self.Page,
                    Section = Self,
                ...
                if Params.Parent then
                    ...
                    Parent = Card.Section.Items["Content"]
                end
                ...
                local Items = { } do
                    ...
                    -- headshot image
                    Items["Headshot"] = Library:Create("ImageLabel", {
                        Name = "\0",
                        Parent = Items["Card"].Instance,
                    ...
                    Library:Create("UICorner", {
                        Name = "\0",
                        Parent = Items["Headshot"].Instance,
                        CornerRadius = UDim.new(0, 4)
                    })

                    pcall(function()
                        Items["Headshot"].Instance.Image = "rbxthumb://type=AvatarHeadShot&id=" .. Card.UserId .. "&w=48&h=48"
                    end)
                    ...
                    Library:Create("UICorner", {
                        Name = "\0",
                        Parent = Items["HPBar"].Instance,
                        CornerRadius = UDim.new(0, 2)
                    })

                    -- health bar fill
                    Items["HPFill"] = Library:Create("Frame", {
                        Name = "\0",
                        Parent = Items["HPBar"].Instance,
                        Size = UDim2.new(1, 0, 1, 0),
                        BorderSizePixel = 0,
                    ...
                    -- chat label (recent chats)
                    Items["Chat"] = Library:Create("TextLabel", {
                        Name = "\0",
                        Parent = Items["Card"].Instance,
                        FontFace = Library.Font,
                        TextSize = 9,
                    ...
                    Library:Create("UIStroke", {
                        Name = "\0",
                        Parent = Items["Chat"].Instance
                    })

                    -- selection stroke
                    Items["SelectStroke"] = Library:Create("UIStroke", {
                        Name = "\0",
                        Parent = Items["Card"].Instance,
                        ApplyStrokeMode = Enum.ApplyStrokeMode.Border,
                        LineJoinMode = Enum.LineJoinMode.Miter,
                    ...
                    -- hover effect
                    Items["Card"]:OnHover(function()
                        Items["Card"]:Tween({BackgroundColor3 = Library.Theme["Hovered Element"]})
                    end, function()
                        Items["Card"]:Tween({BackgroundColor3 = Library.Theme["Content"]})
                    end)

                    Card.Items = Items
                end

                function Card:SetSelected(Bool)
                    Items["SelectStroke"].Instance.Thickness = Bool and 1 or 0
                end

                function Card:SetHealth(ratio)
                    ratio = math.clamp(ratio, 0, 1)
                    Items["HPFill"].Instance.Size = UDim2.new(ratio, 0, 1, 0)
                    if ratio > 0.5 then
                        Items["HPFill"].Instance.BackgroundColor3 = Color3.fromRGB(0, 255, 0):Lerp(Color3.fromRGB(255, 255, 0), (1 - ratio) * 2)
                    else
                        Items["HPFill"].Instance.BackgroundColor3 = Color3.fromRGB(255, 255, 0):Lerp(Color3.fromRGB(255, 0, 0), (0.5 - ratio) * 2)
                    end
                end

                function Card:SetDistance(text)
                    Items["Distance"].Instance.Text = text
                end

                function Card:SetChat(text)
                    Items["Chat"].Instance.Text = text
                end

                function Card:SetVisibility(Bool)
                    Items["Card"].Instance.Visible = Bool
                end

                function Card:Destroy()
                    pcall(function() Items["Card"].Instance:Destroy() end)
                end

                Items["Card"]:Connect("MouseButton1Down", function()
                    Library:SafeCall(Card.Callback)
                end)

                return setmetatable(Card, Library)
            end
